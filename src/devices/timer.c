...
bool wakeup_time_less(const struct list_elem *a, const struct list_elem *b, void *aux UNUSED) {
  const struct thread *t1 = list_entry(a, struct thread, sleepelem);
  const struct thread *t2 = list_entry(b, struct thread, sleepelem);
...
timer_sleep (int64_t ticks) 
{
  ...
  cur_thread->wakeup_time = wakeup_time;

  enum intr_level old_level = intr_disable();
  list_insert_ordered(&sleeping_list, &cur_thread->sleepelem, wakeup_time_less, NULL);
  thread_block();
  intr_set_level(old_level);
}
